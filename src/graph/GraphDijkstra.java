...
public class GraphDijkstra{   
    ...
    private void updateQueue(PriorityQueue queue, int nodeId, int oldDist, int newDist){
        ...
        // This is the first step for updating new shortest possible distance queue.remove(new QueueNode(nodeId, oldDist));
        ...
        // if queue is not updated then that means entry for this node does not exist and has to be added now 
        if (!queueUpdated){
    ...
    public int[] findShortestDijkstra (int srcId){
        ...
        while (!queue.isEmpty()){
            // greedy approach -  remove the first node which would have least distance from source among all the nodes in queue
            QueueNode currQueueNode = queue.remove();
            visited[currQueueNode.nodeId] = true;
            ...
            while (neighborNode != null) {
                if (!visited[neighborNode.nodeId]) {
