...
public class BFStraversalAllPathsSrcDestAdjacencyList {
    ...
    public static void main(String[] args){
    	...
    	BFStraversalAllPathsSrcDestAdjacencyList app = new BFStraversalAllPathsSrcDestAdjacencyList(6);
        app.addEdge(0, 4)
        .addEdge(0, 1)
        .addEdge(1, 2)
        .addEdge(1, 4)
    ...
    public BFStraversalAllPathsSrcDestAdjacencyList(int numberOfNodes) {
    ...
    public BFStraversalAllPathsSrcDestAdjacencyList addEdge(int from, int to) {
        adj[from].add(to);
        //unless unidirectional: //if a is connected to b than b should be connected to a
        adj[to].add(from);
    ...
    public void findPath(int source, int destination) {
        ...
        while (!queue.isEmpty()) {
            ...
            visited[src] = true;
            if (src == destination) {
                System.out.println("Path from "+source+" to " + destination+ " :- "+ pathToNode[src]);
    ...
    public void findAllpaths(int source, int destination) {
        ...
        while(! allVisited() && !queue.isEmpty()) {
            ...
            while (!queue.isEmpty()) {
                ...
                visited[src] = true;
                if (src == destination) {
                    System.out.println("Path " + ++pathCounter + " from "+source+" to " + destination+ " :- "+ pathToNode[src]);
                    //mark nodes that are included in the path, so they will not be included in any other path
                ...
                while (i.hasNext()) {
                    int n = i.next();
                    if (! visited[n] && ! queue.contains(n) && ! includedInPath[n] /*ignore nodes already in a path*/) {
                        queue.add(n);
                        pathToNode[n].addAll(pathToNode[src]);
                        pathToNode[n].add(src);
                    }
